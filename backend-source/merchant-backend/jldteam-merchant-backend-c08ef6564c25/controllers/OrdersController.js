...
const validate = require('../helper/validators/OrdersValidator');
const fs = require('fs')
...
const OrdersController = () => {
    ...
    const ordersList = async(req, res) => {
        try {
            ...
            const { user } = req;
            const validationResponse = validate.ordersList(req.body);
            ...
            var OrderData = await Order.findAll({
                ...
                where: { order_type: req.body.type },
                order: [
                    ['id', 'DESC'],
    ...
    const updateordertransactionid = async(req, res) => {
        try {
            ...
            let getorder = await Order.findOne({ where: { id: body.order_id } });

            if (getorder) {


                let updateorderdata = await Order.update({
                        status: body.status,
                ...
                if (updateorderdata[0] === 1) {
                    response.successMsg(res, constant.ORDER_STATUS_UPDATED);
                } else {
                    return response.error(res, constant.ORDER_STATUS_NOTUPDATED);
                }
    ...
    const updateorderstatus = async(req, res) => {
        try {
            ...
            const { user } = req;
            // const confirmPassword = req.body.confirmPassword
            const validationResponse = validate.updateorderstatus(body);
            if (!validationResponse.status) {
                return response.bodyNotFound(res, validationResponse.msg);
            }
